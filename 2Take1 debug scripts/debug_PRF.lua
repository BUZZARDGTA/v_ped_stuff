...
local SCRIPT_NAME <const> = "debug_PRF.lua"
local SCRIPT_TITLE <const> = "DEBUG (PRF)"
...
max_Feat.hint = "The max flag value. (max-known one is 459)"
max_Feat.min = 0
max_Feat.max = 1000
max_Feat.value = 1000
...
create_tick_handler(function()
    ...
    if enable_Feat.on then
        for i = min_Feat.value, max_Feat.value do
            NATIVES.PED.SET_PED_RESET_FLAG(playerPed, i, (boolean_Feat:get_str_data()[boolean_Feat.value + 1] == "true"))
        end
